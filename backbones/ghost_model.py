...
def ghost_module(inputs, out, convkernel=1, dwkernel=3, add_activation=True):
    ...
    cc = Conv2D(conv_out_channel, convkernel, use_bias=False, strides=(1, 1), padding="same",
    ...
    nn = DepthwiseConv2D(dwkernel, 1, padding="same", use_bias=False, depthwise_initializer=CONV_KERNEL_INITIALIZER)(
        cc)  # padding=dw_size//2
...
def ghost_bottleneck(inputs, dwkernel, strides, exp, out, se_ratio=0, shortcut=True):
    ...
    if strides > 1:
        ...
        nn = DepthwiseConv2D(dwkernel, strides, padding="same", use_bias=False,
                             depthwise_initializer=CONV_KERNEL_INITIALIZER)(nn)
        nn = BatchNormalization(axis=-1)(nn)
        # nn = Activation('relu')(nn)
    ...
    if shortcut:
        xx = DepthwiseConv2D(dwkernel, strides, padding="same", use_bias=False,
                             depthwise_initializer=CONV_KERNEL_INITIALIZER)(
            inputs
        )  # padding=(dw_kernel_size-1)//2
        xx = BatchNormalization(axis=-1)(xx)
        xx = Conv2D(out, (1, 1), strides=(1, 1), padding="valid", use_bias=False,
                    kernel_initializer=CONV_KERNEL_INITIALIZER)(xx)  # padding=0
        xx = BatchNormalization(axis=-1)(xx)
    else:
        xx = inputs
    return Add()([xx, nn])
...
def GhostNet(input_shape=(224, 224, 3), include_top=True, classes=0, width=1.3, strides=2, name="GhostNet"):
    ...
    nn = Conv2D(out_channel, (3, 3), strides=strides, padding="same", use_bias=False,
                kernel_initializer=CONV_KERNEL_INITIALIZER)(inputs)  # padding=1
    ...
    nn = Conv2D(out, (1, 1), strides=(1, 1), padding="valid", use_bias=False,
    ...
    if include_top:
        ...
        nn = Conv2D(1280, (1, 1), strides=(1, 1), padding="same", use_bias=False,
                    kernel_initializer=CONV_KERNEL_INITIALIZER)(nn)
        nn = BatchNormalization(axis=-1)(nn)
        nn = activation(nn)

        # Head containing features with the same number of classes
        nn = Conv2D(classes, (1, 1), strides=(1, 1), padding="same", use_bias=False,
                    kernel_initializer=CONV_KERNEL_INITIALIZER)(nn)
